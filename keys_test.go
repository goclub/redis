...
func TestUnlink(t *testing.T) {
	for _, client := range Connecters {
		redisUnlink(t, client)
	}
}
func redisUnlink(t *testing.T, client Connecter) {
	ctx := context.TODO()
	key := "unlink"
	key2 := "unlink2"
	{// SET key v
	...
	{// UNLINK key
		delCount, err := UNLINK{Key: key}.Do(ctx, client)
		assert.NoError(t, err)
		assert.Equal(t, delCount, uint64(1))
		reply, isNil, err := client.DoStringReply(ctx, []string{"GET", key})
		assert.NoError(t, err)
	...
	{// UNLINK key key2
		delCount, err := UNLINK{Keys: []string{key, key2}}.Do(ctx, client)
		assert.Equal(t, delCount, uint64(2))
		assert.NoError(t, err)
		// GET key
		{
